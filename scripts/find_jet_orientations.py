...
# Load data.
for sampleLabel, sampleProperties in SAMPLES.items():
    print("=" * 60)
    print(f"Sample '{sampleLabel}'")

    namesColumn   = sampleProperties["namesColumn"]
    pathCatalogue = DIR_INPUT / "catalogues" / sampleProperties["nameCatalogue"]
    pathImages    = DIR_INPUT / sampleProperties["nameDirectoryImages"]

    with fits.open(pathCatalogue) as hduList:
        data                    = hduList[1].data
        go_right_ascensions     = data[namesColumn["rightAscension"]] # in deg
        go_declinations         = data[namesColumn["declination"]]    # in deg
        go_length_angular_means = data[namesColumn["lengthAngular"]]  # in arcmin
        go_redshift_means       = data[namesColumn["redshift"]]       # in 1
    ...
    for filename in sorted(os.listdir(pathImages)):
    ...
    for filename in filenames:
        ...
        if OVERWRITE_FILES or not check_if_file_exists_in_correct_location(adjustment_status, save_a, save_sa, save_m):
            ...
            jet_system = JetSystem(file_path, ra, dec, image_width_arcmins, adjustment_status, SA_LIST, M_LIST, angles,
                # Data
